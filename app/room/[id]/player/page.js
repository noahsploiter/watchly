...
function PlayerRoomContent({ params }) {
  const { user } = useAuth();
  ...
  return <VideoPlayer room={room} isHost={isHost} onRoomUpdate={fetchRoom} />;
}

export default function PlayerRoomPage({ params }) {
  return (
    <AuthProvider>
      <PlayerRoomContent params={params} />
    </AuthProvider>
  );
}
